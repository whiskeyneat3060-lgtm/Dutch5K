...
2. **Meanings + examples for GENERAL 5K words — IN PROGRESS** (hand-written into `GENEX`, no API). Of the
   ...
   worklist, rank-sorted). **Done so far: through batch 21 (cache v40); deck-with-examples ≈ 5,144;
   ~451 general words remain.** Method per batch (~150 words): `node rework.js` to regenerate the
